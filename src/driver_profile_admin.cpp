...
int diagnoseMedia(const Args&a){auto image=cv::imread(required(a,"input"));if(image.empty())throw std::runtime_error("unable to decode diagnostic input");dms::OpenCvSFaceProvider sface(required(a,"detector"),required(a,"recognizer"),"opencv-sface-2021dec-evaluation");dms::OpenCvAntiSpoofProvider pad(required(a,"pad-model"),"omz-anti-spoof-mn3-evaluation");dms::OpenCvDiagnosticFaceQualityProvider quality;if(!sface.valid())throw std::runtime_error(sface.diagnostic());if(!pad.valid())throw std::runtime_error(pad.diagnostic());dms::DriverEnrollmentProcessor processor({0.0F,false},sface,quality,pad,sface);dms::FaceImageView source{image.data,image.cols,image.rows,static_cast<std::ptrdiff_t>(image.step)};auto result=processor.evaluate(source);std::cout<<"{\"schema\":2,\"diagnostic_only\":true,\"enrollment_allowed\":false,\"decision\":"<<static_cast<int>(result.decision)<<",\"quality_score\":"<<result.quality<<",\"pad_state\":"<<static_cast<int>(result.presentation.state)<<",\"pad_score\":"<<result.presentation.score<<",\"pad_thresholds_approved\":false,\"embedding_dimensions\":"<<result.embedding.values.size()<<",\"embedding_model\":\""<<result.embedding.modelId<<"\"}\n";return result.decision==dms::EnrollmentDecision::ThresholdApprovalRequired?0:1;}
}
